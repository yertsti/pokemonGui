...
* @author Lisa Miller, Kenneth Seekins
* @since 8/18/18
*/
public abstract class Pokemon implements Comparable<Pokemon> {
   ...
   *@throws PokemonException for blank or space only names.
   **/
   public Pokemon(String species, String name, 
      ...
      if (s1.length() > 0) {
         ...
         throw new PokemonException("Pokemon Name may not be blank nor just space(s)");
      } //close else
   ...
   * @throws PokemonException if name is blank or space(s) only. 
   */
   public void setName(String s) throws PokemonException {
      String s1 = s.trim();
      if (s1.length() > 0) {
         this.name = s1;
      } else {
   ...
   */  
   ...
   /**
	 * Compares a Pokemon to another Pokemon.
	 *            
   ...
   */
   public int compareTo(Pokemon poke) {
      int diff = 0;
      diff = this.getNumber() - poke.getNumber();
      if (diff == 0) {
         diff = this.getName().toLowerCase().compareTo(poke.getName().toLowerCase()); 
      } 
      if (diff == 0) {
         diff = this.getHP() - poke.getHP();
      } 
      if (diff == 0) {
         diff = this.getCP() - poke.getCP();
      } 
      if (diff == 0) {
         diff = this.getFastAttack().toLowerCase().compareTo(poke.getFastAttack().toLowerCase());
      } 
      if (diff == 0) {
         diff = this.getSpecialAttack().toLowerCase().
            compareTo(poke.getSpecialAttack().toLowerCase());
      }
   ...
   /**
	 * Checks if 2 Pokemons are equal.
	 *            
   * @return boolean true if equal, false if not. 
   *
   * @param poke second Pokemon
   *    
   */
   public boolean equalTo(Pokemon poke) {
